...
class DbHelper(context: Context?) : SQLiteOpenHelper(
    ...
    private fun runMigrations(db: SQLiteDatabase) {
        val migrationHistory: List<MigrationHistoryEntity> = MigrationHistoryDao().findAll(db).sortedBy { it.version }
        ...
        val mapped = sorted.map { Pair(it.split("_").first().removePrefix(DbConfig.MIGRATION_PREFIX).toInt(), it) }
        for (i in mapped) {
            ...
            val migrationClass: Class<AbstractMigration> = Class.forName("${DbConfig.MIGRATION_PACKAGE}.${i.second}") as Class<AbstractMigration>
