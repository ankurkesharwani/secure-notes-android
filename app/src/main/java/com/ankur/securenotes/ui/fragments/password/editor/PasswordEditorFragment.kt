...
class PasswordEditorFragment : Fragment(), PasswordEditorFragmentManager.Listener,
    ...
    fun savePassword() {
        ...
        if (title.isEmpty()) {
            listener?.get()
                ?.onPasswordSavingFailed(null, "Error: Cannot save an password without a title.",
                    WeakReference(this))
        ...
        if (url.isEmpty() && email.isEmpty() && phone.isEmpty() && username.isEmpty()) {
            listener?.get()?.onPasswordSavingFailed(null,
    ...
    private fun setDependencies(savedInstanceState: Bundle?) {
        manager = if (savedInstanceState?.getBoolean("isChangingConfiguration") == true) {
            ...
            context?.let {
                PasswordEditorFragmentManagerBuilder().set(context = activity).set(listener = this)
                    .build()
            }!!
        }
    }
    ...
    private fun updateUiState() {
        ...
        binding.etPhoneEditText.isEnabled = false
        binding.etUsernameEditText.isEnabled = false
